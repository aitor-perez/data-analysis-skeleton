...
**Methodological choices.** *Flag key analytical choices that affect interpretation.*

**Scope and intent.** *Clarify what this dashboard is and is not.*
:::

**Last updated:** `{python} from datetime import date; date.today().strftime("%d %B %Y")`

**Data pipeline:** All numbers are loaded from analysis outputs. No values are hardcoded. To refresh, re-run the underlying analyses and re-render.
