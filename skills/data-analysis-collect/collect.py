...
original_dir = project_dir / "1_data" / "original"
sources_path = original_dir / "sources.yaml"
skill_dir = Path(__file__).parent.resolve()
template_path = skill_dir / "templates" / "sources.yaml"
...
if not sources_path.exists():
    ...
    print(f"✓ Created {sources_path.relative_to(project_dir)} from template")
...
disk_files = collect_data_files(original_dir)
...
if not disk_files and not documented_files:
    print("⚠ No raw data files found in 1_data/original/.")
else:
    print(f"✓ {len(disk_files)} file(s) in 1_data/original/")
    print(f"✓ {len(documented_files)} file(s) documented in sources.yaml")
